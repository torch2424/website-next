(function (global, factory) {
	...
	*/
	...
	unwrapExports(scheduleTracing_production_min);
	var scheduleTracing_production_min_1 = scheduleTracing_production_min.__interactionsRef;
	var scheduleTracing_production_min_2 = scheduleTracing_production_min.__subscriberRef;
	var scheduleTracing_production_min_3 = scheduleTracing_production_min.unstable_clear;
	var scheduleTracing_production_min_4 = scheduleTracing_production_min.unstable_getCurrent;
	var scheduleTracing_production_min_5 = scheduleTracing_production_min.unstable_getThreadID;
	var scheduleTracing_production_min_6 = scheduleTracing_production_min.unstable_trace;
	var scheduleTracing_production_min_7 = scheduleTracing_production_min.unstable_wrap;
	var scheduleTracing_production_min_8 = scheduleTracing_production_min.unstable_subscribe;
	var scheduleTracing_production_min_9 = scheduleTracing_production_min.unstable_unsubscribe;
	...
	{
	  module.exports = scheduleTracing_production_min;
	}
	});
	...
	function gameOfLife(canvasElement, wasmPath) {
	  // Configuration
	  const RGB_ALIVE = 0xd392e6;
	  const RGB_DEAD = 0xa61b85;
	  const BIT_ROT = 10; // Set up the canvas with a 2D rendering context

	  var cnv = canvasElement;
	  var ctx = cnv.getContext("2d");
	  var bcr = cnv.getBoundingClientRect(); // Compute the size of the universe (here: 2px per cell)
	  ...
	  fetch(wasmPath).then(response => response.arrayBuffer()).then(buffer => WebAssembly.instantiate(buffer, {
	    env: {
	      memory,
	      abort: function () {}
	    },
	    ...
	    var mem = new Uint32Array(memory.buffer); // Update about 30 times a second
	    ...
	    var down = false;
	    [[cnv, "mousedown"], [cnv, "touchstart"]].forEach(eh => eh[0].addEventListener(eh[1], e => down = true));
	    [[document, "mouseup"], [document, "touchend"]].forEach(eh => eh[0].addEventListener(eh[1], e => down = false));
	    [[cnv, "mousemove"], [cnv, "touchmove"], [cnv, "mousedown"]].forEach(eh => eh[0].addEventListener(eh[1], e => {
	      if (!down) return;
	      var loc;
	...
	class GameOfLife extends react.Component {
	  ...
	  render() {
	    const divStyles = {
	      ...
	      'margin-right': 'auto',
	      'margin-top': '10px',
	      'margin-bottom': '10px' // Styles for the game of life canvas
	...
	function render(containerElement, wasmPath) {
